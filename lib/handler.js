...
export async function handleMessage(sock, m, commands) {
    ...
    // --- Command Execution ---
    try {
        const groupName = isGroup ? groupMetadata.subject : 'Direct Message';
        const logMessage = `[CMD] ${prefix}${command.name} | User: ${senderNumber} | Group: ${groupName}`;
        console.log(Fg('magenta')(logMessage));
