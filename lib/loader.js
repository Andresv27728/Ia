...
export async function loadCommandsAsync() {
    ...
    console.log(Fg('blue')('─'.repeat(20)));
    console.log(Fg('cyan')('  LOADING COMMANDS'));
    console.log(Fg('blue')('─'.repeat(20)));
        ...
        for (const file of commandFiles) {
            ...
            const promise = import(`file://${filePath}`).then(module => {
                ...
                if (command && command.name) {
                    command.category = category;
                    commands.set(command.name, command);
                    // More subtle log for each command
                    console.log(Fg('gray')(`  └── Loaded ${command.name}`));
                } else {
                     console.log(Fg('yellow')(`[WARNING] Command in ${file} is missing 'name' or default export.`));
                }
            }).catch(err => {
    ...
    console.log(Fg('blue')('─'.repeat(20)));
    console.log(Fg('cyan')(`  LOADED ${commands.size} COMMANDS`));
    console.log(Fg('blue')('─'.repeat(20) + '\n'));
    return commands;
}
