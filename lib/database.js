...
const databasePath = path.join(process.cwd(), 'lib', 'database');
...
const initDatabase = async () => {
    try {
        ...
        // Load existing data for Maps
        const files = ['users.json', 'groups.json', 'premium.json', 'settings.json'];
        ...
        files.forEach(file => {
            ...
            if (fs.existsSync(filePath)) {
                ...
                if (databases[dbName]) {
                    databases[dbName] = new Map(Object.entries(data));
                }
            }
        });
...
const saveDatabase = async () => {
    try {
        ...
        // Save Map databases
        for (const [dbName, dbMap] of Object.entries(databases)) {
            const filePath = path.join(databasePath, `${dbName}.json`);
            const data = Object.fromEntries(dbMap);
...
const updatePremiumStatus = async (id, data = {}) => {
    ...
    // Also update in Map database
    databases.premium.set(id, premium[id]);
 ...
 */
const isPremium = (id) => {
    if (!premium[id] && !databases.premium.has(id)) return false;
    
    const premiumData = premium[id] || databases.premium.get(id);
    const now = moment().unix();
    return premiumData && premiumData.expiration > now;
};
...
const getPremiumInfo = (id) => {
    ...
    const info = premium[id] || databases.premium.get(id);
    const now = moment().unix();
