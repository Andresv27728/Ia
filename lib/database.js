 ...
 */
const checkPremiumExpiration = async (sock) => {
    if (!sock) return; // Do nothing if sock is not provided
    ...
    for (const id in premium) {
        const info = premium[id];
        const userJid = `${id.split('@')[0]}@s.whatsapp.net`;

        // Remove expired premium users
        if (info.expiration <= now) {
            delete premium[id];
            logger.info(`Premium expired for ${id} and was removed.`);
            try {
                await sock.sendMessage(userJid, { text: `😔 Your premium subscription has expired. Please contact the owner to renew.` });
            } catch (e) {
                logger.error(`Failed to send expiration message to ${id}:`, e);
            }
            changesMade = true;
            continue;
        }
        ...
        // Check if expiration is within notification threshold
        ...
        if (timeToExpiration > 0 && timeToExpiration <= notificationThreshold) {
            ...
            const days = Math.floor(timeToExpiration / 86400);
            const hours = Math.floor((timeToExpiration % 86400) / 3600);

            const remainingText = days > 0 ? `${days} day(s) and ${hours} hour(s)` : `${hours} hour(s)`;
            
            logger.info(`Sending premium expiration notification to ${id}: ${remainingText} remaining`);
            
            try {
                const message = `⚠️ Your premium subscription is expiring soon!\n\n*Time Remaining:* ${remainingText}\n\nPlease contact the owner to renew.`;
                await sock.sendMessage(userJid, { text: message });
            } catch (e) {
                logger.error(`Failed to send expiration reminder to ${id}:`, e);
                // If sending fails, undo notification flag to try again later
                premium[id].notified = false;
            }
        }
    }
