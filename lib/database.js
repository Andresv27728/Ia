...
const isPremium = async (userId) => {
    return databases.premium.has(userId);
};

const getPremiumInfo = async (userId) => {
    return databases.premium.get(userId) || null;
};

const updatePremiumStatus = async (userId, data) => {
    databases.premium.set(userId, data);
};
